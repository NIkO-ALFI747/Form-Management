...
public class UsersRepository(FormManagementDbContext context, ILogger<UsersRepository> logger) : IUsersRepository
{
    ...
    public async Task<Result<bool, Error>> AddAsync(User user, CancellationToken cancellationToken)
    {
        try
        {
            await PerformAddOperation(user, cancellationToken);
            return true;
        }
    ...
    private async Task PerformAddOperation(User user, CancellationToken cancellationToken)
    {
        await _context.Users.AddAsync(user, cancellationToken);
        await _context.SaveChangesAsync(cancellationToken);
    }

    private Result<bool, Error> HandleDbUpdateException(DbUpdateException ex)
    {
    ...
    private Result<bool, Error> HandleUniqueViolation(PostgresException postgresEx)
    {
